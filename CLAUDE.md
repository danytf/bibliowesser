...
- `#vfsCtl` fades out (`.oculto`) `CTL_MS` after playback starts — 1s, chosen by the user after seeing it on the device — and comes back on any pointer/key activity over `#vfs`, via `mostrarCtl()`. Every interaction restarts the timer, so a tap always buys another second. It deliberately stays put while paused, and `#vfsClose` never hides — on a kiosk tablet the way out must always be on screen. Note the listener is on `#vfs` in the capture phase: the shield eats the tap before anything else sees it, so this is the only way to get the bar back on a touch device.
- Captions are forced off: `cc_load_policy=0` on the embed is not enough on its own (a user/browser preference still wins), so `quitarSubtitulos()` also sends `unloadModule` for both `captions` and `cc` on every state change. This only removes YouTube's caption track — a few videos (e.g. AECC's "el testimonio de Víctor") have subtitles burned into the image and nothing can turn those off.
- The dialog is a real focus trap: `vfsKeydown` handles `Tab`/`Shift+Tab` and cycles through `vfsFocos()` only, so focus never reaches the ficha behind or the YouTube iframe — `aria-modal="true"` would otherwise be a promise the page does not keep. On close, focus returns to the link that opened the video, and it has to happen *after* `exitFullscreen()` resolves, because leaving fullscreen hands focus back to the document and overwrites any earlier `focus()`.
...
1. **`<style>` (head)** — all CSS, using CSS custom properties defined on `:root` for dark mode and overridden under `body.light` for light mode (toggled via `#thbtn`, persisted to `localStorage` key `wt`). **Light is the default**: `class="light"` ships on `<body>` itself, so the page is correct even before any script runs, and the inline script only *removes* it when `wt === '0'` (an explicit choice of dark). `wt` is `'1'` light, `'0'` dark, absent = never touched. Because the static markup is the light state, the `DOMContentLoaded` sync of `#thbtn` has to set both branches, not just the light one. Fonts: Plus Jakarta Sans (body text) and Space Grotesk (`--font-display`, used for numbers/headings).
